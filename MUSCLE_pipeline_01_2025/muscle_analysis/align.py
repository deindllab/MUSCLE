...
def peaks_threshold (path_smFRET, POS, ALEX, tr_R2G):
    ...
    path_smFRET_file = []
    while not path_smFRET_file:
        pos_idx = int(input("Please enter the number of position that you want to use:"))
        pos = labels[pos_idx]
    ...
    while flag:
            thr = int(input("Please, choose the threshold, default value 300: "))
            fig, ax = plt.subplots()
            ax.imshow(combined)
...
def check_image_correlation(img1, img2):
    """
    Computes the correlation coefficient between two images.
    
    Returns:
        float: Correlation coefficient between the two images.
    """
...
def alignment (path_smFRET,POS, ALEX, tr_R2G, x_coord, y_coord, sequence, read2, thr = 300, show_images = False):
    global mdict1
    """
    ...
    Args:
        ...
        show_images: flag that controls whether the images are displayed during alignment. Displaying images significantly slows down the alignment process
    Returns:
    ...
    for pos in labels_res:
        ...
        for current_dir, dirs, files in os.walk(os.path.join(path_smFRET,pos)) :
            ...
            break
        if bool(path_smFRET_file) & (not os.path.exists(os.path.join(current_direct, pos + "_traces.mat"))) & (not pos in processed_pos):
            ...
            if show_images:
                fig, ax = plt.subplots()
                print("Now showing the combined image")
                ax.imshow(combined)
            print("thr: "+str(thr))
            ...
            if show_images:       
                ...
                print("Now showing the detected smFRET Peaks")
                plt.show()
            ...
            if show_images:
                ...
                print("Now showing the transformed smFRET peaks")
                plt.show()
            ...
            if show_images:
                fig, ax = plt.subplots()
                print("Now showing the fastq library array")
            ...
            for blob in FQ_centers_matched:
                c = plt.Circle(blob, 3, color="blue", linewidth=1, fill=False)
                ax.add_patch(c)
            ax.set_axis_off()
