...
def get_translation():
    ...
    # Selecting a small part of the template 
...
#orient the stage coordinate to image coordinate
def combined_image(path_smFRET, POS, ALEX, tr_R2G, tr = 300):
    ...
    Args:
        ...
        orientation: should be intergers from -4 to 4 (excluding 0) if the input is positive, rotation [1 (0) 2 (90) 3(180) 4(270)] will be performed on the image before combining. If the input is negative, the same angular rotation will be perform on the flip left/right of the image 
    ...
    """
    size1 = apriori_tr_original([[red_y_end-red_y_start,red_x_end-red_x_start]]).astype(int)
    labels, posX, posY  = load_data.extract_pos_info(POS, False) #from module load_data_extracting the coordinates and the label from the position, if False, then just extract, if True choose matching 
    minx = min(posX)
    maxx = max(posX)
    miny = min(posY)
    maxy = max(posY)
    ...
    print("miny"+str(miny))
    print("minx"+str(minx))
    size = [maxy-miny+50+size1[0,0]*MiSeq_pixel*0.001, maxx-minx+50+size1[0,1]*MiSeq_pixel*0.001] #Size of the combined image in micrometers 0.34 is the ratio between illumna pixel variable and 1 micron pixel scale
    size = np.divide(size,MiSeq_pixel*0.001).astype(int) # Size of the combined image in MiSeq pixels
    img = np.zeros(size)
    counter = 1
        ...
        for current_dir, dirs, files in os.walk(os.path.join(path_smFRET,pos)) :
            for el in files:
                if (el.split('.')[-1].lower() == 'tif') or (el.split('.')[-1].lower() == 'tiff'):
                    path1 = os.path.join(current_dir,el)
                    break
            else: continue
            break
        if not path1:
...
#         img_t = np.mean(img_smFRET[green_frames,::], axis = 0)
        ...
        red = img_t[red_y_start:red_y_end,red_x_start:red_x_end]
        
        green = img_t[green_y_start:green_y_end,green_x_start:green_x_end]
    ...
    #     plt.savefig(os.path.join(pos_direct, pos + "_smFRET_peaks.tif"))
        ...
        id1 = int((y1[0]-miny)*1000/MiSeq_pixel)
        id2 = int((x1[0]-minx)*1000/MiSeq_pixel)
