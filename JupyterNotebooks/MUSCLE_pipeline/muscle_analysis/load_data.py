...
def poly_transf():
    """This function is used for creating the polynomial transformation and it's further usage in code
    ...
    file_path = fd.askopenfilename(title = "Choose the forward transform file")
    tr_G2R.params = np.load(file_path)
    tr_R2G = transform.PolynomialTransform()
    file_path = fd.askopenfilename(title = "Choose the inverse transform file")
