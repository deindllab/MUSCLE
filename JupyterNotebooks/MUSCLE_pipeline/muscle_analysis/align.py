...
def alignment (path_smFRET, POS, ALEX, tr_R2G, apriori_tr, x_coord, y_coord, sequence, read2 ):
    ...
    Returns:
        The function is used to choose the good positions (ones which were aligned to FASTQ data). It returns the path to the file good_pos.mat with selected positions and the detailed data about the alignment.
...
#             img_t = np.mean(img_smFRET[green_frames,::], axis = 0)
    ...
    return (os.path.join(current_direct, "good_pos.mat"))